...
| `CBM_SCIP_AUTO_DISCOVER` | unset (off) | Persistence gate (2026-07-27). `CBM_SCIP_INDEX_PATH` is a SINGLE global path applied to whatever repo is being indexed, so it cannot persist the tier across more than one repo — exporting it in a launcher points every repo at one repo's index (a silent no-op; the drift guard excludes non-matching documents). Set this to `1` and the pass uses **`<repo>/index.scip`** when present, so the measured gain survives a re-index per-repo. Deliberately NOT on by default: treating "a file named `index.scip` exists" as consent would let a stale index silently start rewriting CALLS edges with no operator action — `TestSCIPIngestInertWithoutEnv` pins the existing env-var-only contract and still passes. Precedence: `CBM_SCIP_INDEX_PATH` > discovered `<repo>/index.scip` (when this is on) > off. Provenance is logged as `pass.scip_ingest.selected` with `source=env:…` or `source=repo-default:index.scip`. Tests: `internal/pipeline/scip_index_path_test.go`. |
