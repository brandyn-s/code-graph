...
> **Reproduction status (2026-06-12)**: re-baselined on the current engine (main `f72d07e`, after #371–#389) over the **same 200 instances** as the 2026-05-04 run — **clean reproduction**: file=85.5% / class=84.0% / func=76.5%, paired 10K-resample bootstrap Δ all include zero (file +0.0 [−3.0,+3.0], class −0.5 [−4.0,+3.0], func +3.0 [−1.5,+8.0]). **200/200 cloned and indexed, 0 failures** — the 2026-05-12 "58 GC'd commits" blocker is dead (it was the `ls-remote`-vs-ancestor instrument error; the corpus is fully reachable via the commits API). The 86.0/84.5/73.5 defended numbers stand, now confirmed on the reachable corpus and **safe to cite externally**. The Bug Report dominant-miss cell was classified REAL-with-stochastic-component (temperature-1.0 run-to-run variance, ~2/3 of sampled misses flip on re-run), not an instrument artifact. Details: `bench/accuracy/baselines/2026-06-12-loc-bench-n200-rebaseline.md`. (Superseded the 2026-05-16 REFUSED-for-publication caveat.)
...
| `CBM_SCIP_AUTO_DISCOVER` | unset (off) | Legacy compatibility only. New callers should pass `precision_tier="scip"` to `index_repository`; the per-project choice persists and defaults to `<repo>/index.scip`. This environment flag still enables repo-local discovery when no explicit project preference exists. Explicit `precision_tier="heuristic"` suppresses it. |
| `CBM_SCIP_INDEX_PATH` | unset (off) | Legacy compatibility only. New callers should use `scip_index_path` with `precision_tier="scip"`. The explicit per-project contract reports the requested/effective tier, SHA-256, document/function coverage, drift, replacements, and insertions through indexing and status responses. Missing or invalid requested SCIP is visible as degraded heuristic operation rather than silent compiler-grade success. SCIP-derived calls still apply only to covered, non-drifted documents; `get_relationship_evidence` binds the exact index SHA-256 per derived edge, and only that artifact-bound edge may satisfy compiler assurance. Historical measurements and production Rust validation remain in `bench/accuracy/baselines/2026-06-11-scip-tunnl-validation.md`. |
...
| `CODE_GRAPH_EMBEDDINGS_TIMEOUT_SEC` | unset (built-in default) | Overall timeout for the embeddings pass when `VOYAGE_API_KEY` is set. |
...
| `CODE_GRAPH_UPDATE_CHANNEL` | `stable` | `stable` follows GitHub's latest non-prerelease; `rc` lets `code-graph update` consider `vX.Y.Z-rc.N` release candidates. |
